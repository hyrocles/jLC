...
public class Config{
	...
	private ArrayList<String> getBalancers(){
    	...
    	NodeList xml_connections = xml_localInformation.getElementsByTagName("connections");
    	for(int x = 0; xml_connections.getLength() > x; x++){
    		...
    		for(int y = 0; xml_connection.getLength() > y; y++){
    			if(xml_connection.item(y).getNodeName() == "connection"){
	    			Node temp_connectionType = xml_connection.item(y).getAttributes().getNamedItem("type");
	        		if(temp_connectionType != null){
	        			if(temp_connectionType.getTextContent().contentEquals("balancer")){
	        				Node temp_ip = xml_connection.item(y).getAttributes().getNamedItem("ip");
	        				Node temp_port = xml_connection.item(y).getAttributes().getNamedItem("port");
	        				temp.add(temp_ip.getTextContent()+":"+temp_port.getTextContent());
	        			}
	        		}
        		}
    		}
	...
	public String getNodeType(){
		if(xml_localInformation.getFirstChild().getNodeName().toLowerCase() == "node"){
			Node attr = xml_localInformation.getFirstChild().getAttributes().getNamedItem("type");
			if(attr != null){
